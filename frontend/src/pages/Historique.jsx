...
const STATUS = {
  approuve: { label: 'Approuvé', cls: 'bg-primary-100 dark:bg-primary-500/10 text-primary-700 dark:text-primary-400', icon: Check },
...
export default function Historique() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-primary-600 dark:text-primary-400 mb-1"><HistoryIcon size={18} /><span className="text-xs font-semibold uppercase tracking-wider">Audit</span></div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <Stat label="Approuvées / exécutées" value={stats.approuves} cls="text-primary-600 dark:text-primary-400" />
      ...
      <div className="inline-flex p-1 rounded-xl bg-slate-100 dark:bg-slate-800 mb-5">
        {FILTERS.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${filter === f.key ? 'bg-white dark:bg-slate-900 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>{f.label}</button>
        ))}
      </div>
            ...
            <motion.div key={d.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.3) }}
              ...
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isCredit ? 'bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-400' : 'bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400'}`}>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400">
                ...
                {d.canal === 'vocal' && <span className="inline-flex items-center gap-1 text-primary-600 dark:text-primary-400"><Mic size={11} /> vocal</span>}
              </div>
            </motion.div>
          )
        })}
