...
const inputCls = 'w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:border-primary-500 focus:ring-2 focus:ring-primary-100 dark:focus:ring-primary-500/20 outline-none transition'
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <form onSubmit={submit} className={`${card} p-6 h-fit`}>
        <h2 className="font-semibold mb-5 flex items-center gap-2"><CreditCard size={18} className="text-primary-500" /> Demande de crédit</h2>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="range" min={12} max={300} step={6} value={dureeMois} onChange={(e) => setDureeMois(Number(e.target.value))} className="w-full accent-primary-500" />
          </div>
          <div>
        ...
        <button type="submit" disabled={loading} className="w-full mt-5 py-3 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition flex items-center justify-center gap-2 disabled:opacity-60">
      ...
      <div className={`${card} p-6`}>
        <h2 className="font-semibold mb-5 flex items-center gap-2"><Cpu size={18} className="text-primary-500" /> Pipeline de vérification</h2>
...
function TransferPanel({ client }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <form onSubmit={submit} className={`${card} p-6 h-fit`}>
        <h2 className="font-semibold mb-5 flex items-center gap-2"><Send size={18} className="text-primary-500" /> Nouveau virement</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">Montant (DH)</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-xs font-medium text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition"
              >
          ...
          {showAddBenef && (
            <div className="rounded-xl border border-primary-200 dark:border-primary-500/30 bg-primary-50/50 dark:bg-primary-500/5 p-4 space-y-3">
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5"><UserPlus size={15} className="text-primary-500" /> Nouveau bénéficiaire</p>
              <input
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex-1 py-2 rounded-lg bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 transition flex items-center justify-center gap-2 disabled:opacity-60"
                >
        ...
        <button type="submit" disabled={loading || !beneficiaire} className="w-full mt-5 py-3 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition flex items-center justify-center gap-2 disabled:opacity-60">
          {loading ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
            ...
            <div className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-800">
              {confirmed ? (
                ...
                <form onSubmit={confirm}>
                  ...
                  {result.devOtp && <p className="text-xs text-center text-slate-400 mt-2">Mode démo — code : <span className="font-mono font-bold text-primary-600 dark:text-primary-400">{result.devOtp}</span></p>}
                  {error && <p className="text-sm text-rose-600 dark:text-rose-400 mt-2">{error}</p>}
                  <button type="submit" disabled={confirming} className="w-full mt-3 py-2.5 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition flex items-center justify-center gap-2 disabled:opacity-60">
...
export default function Operations() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2"><ArrowLeftRight className="text-primary-500" size={24} /> Opérations</h1>
      ...
      <div className="inline-flex p-1 rounded-xl bg-slate-100 dark:bg-slate-800 mb-6">
        {[{ k: 'credit', l: 'Crédit', i: CreditCard }, { k: 'virement', l: 'Virement', i: Send }].map(({ k, l, i: Icon }) => (
          <button key={k} onClick={() => setTab(k)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${tab === k ? 'bg-white dark:bg-slate-900 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
            <Icon size={16} /> {l}
          </button>
        ))}
      </div>
