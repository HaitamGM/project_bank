...
export default function Conversations() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 mb-1"><MessagesSquare size={18} /><span className="text-xs font-semibold uppercase tracking-wider">Historique</span></div>
      ...
      {conversations.length === 0 ? (
        <div className={`${card} p-10 text-center`}>
          <div className="w-14 h-14 mx-auto rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mb-4"><MessagesSquare size={26} /></div>
          ...
          <button onClick={() => navigate('/assistant')} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition">
            ...
            return (
              <motion.div key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.3) }} className={`${card} overflow-hidden`}>
                <button onClick={() => setOpenId(open ? null : c.id)} className="w-full flex items-center gap-3 p-4 text-start hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${vocal ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400'}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-400 flex items-center gap-2 mt-0.5">
                      ...
                      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${vocal ? 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' : 'bg-sky-100 dark:bg-sky-500/15 text-sky-700 dark:text-sky-400'}`}>{vocal ? 'Vocal' : 'Texte'}</span>
                    </p>
                  </div>
                      ...
                      <div className="px-4 pb-4 pt-1 border-t border-slate-100 dark:border-slate-800 space-y-3">
                        {(c.messages || []).map((m, j) => (
                          <div key={j} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${m.role === 'user' ? 'bg-emerald-600 text-white rounded-br-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-bl-sm'}`}>
                              {m.role !== 'user' && <p className="text-[10px] uppercase tracking-wide font-semibold text-emerald-600 dark:text-emerald-400 mb-0.5">BankIA</p>}
                              {m.text}
                            </div>
                          </div>
                        ))}
